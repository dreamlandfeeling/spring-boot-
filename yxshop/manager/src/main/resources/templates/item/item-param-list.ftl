<table class="easyui-datagrid" id="itemParamList" title="商品列表"
       data-options="singleSelect:false,collapsible:true,pagination:true,url:'/item/param/',method:'get',pageSize:30,toolbar:itemParamListToolbar">
    <thead>
        <tr>
        ...
        handler:function(){
        	...
        	$.messager.confirm('确认','确定删除ID为 '+ids+' 的商品规格吗？',function(r){
        	    if (r){
        	    	var params = {"ids":ids};
                    $.ajax({
                        type: "delete", url: "/item/param/"+ids, data: params, success: function (data) {
                            if (data.status == 200) {
