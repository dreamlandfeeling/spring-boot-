...
<form id="orderForm" class="hide" action="/order/create" method="post">
	<input type="hidden" name="payment" value="${totalPrice!?c}"/>
	<input type="hidden" name="paymentType" value="2"/>
	<input type="hidden" name="postFee" value="10"/>
	<#list cartList as cart>
		<input type="hidden" name="orderItems[${cart_index}].itemId" value="${cart.id}"/>
		<input type="hidden" name="orderItems[${cart_index}].num" value="${cart.num }"/>
		<input type="hidden" name="orderItems[${cart_index}].price" value="${cart.price}"/>
		<input type="hidden" name="orderItems[${cart_index}].totalFee" value="${cart.price * cart.num}"/>
			...
			<div class="orderTbl">
				...
				<div class="orderPInfo">
					...
					<div class="pItem pGoods">
						<a href="http://localhost:8081/item/${cart.id?c}" target="_blank">${cart.title }</a>
					</div>
  ...
  <div class="orderTFoot">
    ...
    <div class="submitOrder">
      ...
      <div class="orderAccount">
        ...
        <span class="p">¥</span>
        	<span id="countPrice" class="p">${(totalPrice+10!)?c}
        </span>
      </div>
      <span class="clear"></span>
    </div>
